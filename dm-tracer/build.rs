...
/// This crate has a runtime dependency on artifacts produced by the `dm-tracer-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "dm-tracer-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `dm-tracer-ebpf`, so cargo will automatically rebuild us
        // if `dm-tracer-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `dm-tracer-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("dm-tracer-ebpf");
